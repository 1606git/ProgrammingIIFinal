...
predicted_class = ss_model.predict([person1])

# Generate probability of positive class (=1)
probs = ss_model.predict_proba([person1])
...
# Predict class, given input features
predicted_class2 = ss_model.predict([person2])

# Generate probability of positive class (=1)
probs2 = ss_model.predict_proba([person2])

# Print predicted class and probability
print(f"Predicted class: {predicted_class2[0]}") # 0=not pro-environment, 1=pro-envronment
print(f"Probability that this person is pro-environment: {probs2[0][1]}")
...
# %%
user_inputs = [st.number_input('Age'), st.selectbox('Income Level', [1, 2, 3])]
prediction = ss_model.predict([user_inputs])
probability = ss_model.predict_proba([user_inputs])
